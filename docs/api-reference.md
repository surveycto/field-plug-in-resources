...
* `setFieldMetadata(metadataString)`  
...
* `launchPreconfiguredIntent(callback(error, result))` (Android only)  
    Launches the external app configured via 'ex:' appearance and invokes the `callback` function either with an error or the results from the external app when it returns. The field's current value is passed to the external app with as `value` parameter.
